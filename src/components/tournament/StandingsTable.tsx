...
export function StandingsTable({ matches, participants, format, groupName }: StandingsTableProps) {
  // Walkovers are decided results and carry a forfeit scoreline — excluding them
  // understates played counts and diverges from the code that picks who advances.
  // Auto-generated byes have no scores, though, and would read as a 0-0 draw, so
  // only count a walkover once it has a result recorded.
  const completed = matches.filter(
    (m) =>
      m.status === 'completed' ||
      (m.status === 'walkover' && m.player1_score !== null && m.player2_score !== null)
  )
  ...
  for (const m of completed) {
    ...
    if (p1s > p2s) {
      ...
      // A draw is worth a point in every league-style format, not just 'league'.
      // Awarding 0 here made the table players see disagree with advance-knockout,
      // which awards 1 — so the displayed order could contradict who advanced.
      r1.D++; r2.D++
      r1.Pts += 1
      r2.Pts += 1
    }
